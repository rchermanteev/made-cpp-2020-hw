...
Matrix::Matrix(size_t num_rows, size_t num_cols): num_rows(num_rows), num_cols(num_cols) {
	rows = new Matrix::Row [num_rows];
	for (size_t i = 0; i < num_rows; ++i)
		rows[i] = Matrix::Row(num_cols);
...
Matrix::Matrix(const Matrix& copy): num_cols(copy.num_cols), num_rows(copy.num_rows) {
	rows = new Matrix::Row [num_rows];
	for (size_t i = 0; i < num_rows; ++i)
		rows[i] = Matrix::Row(num_cols);
